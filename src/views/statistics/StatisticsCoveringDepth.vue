...
export default {
    ...
    mounted() {
        ...
        getField(this, function (res) {
            ...
            self.eField = getNameNoIgnoreCase(res, self.eField, "1", true);
        });
            ...
            Promise.all(postArr.map(queryFunc))
                .then(function (res) {
                    ...
                    for (let k = 0; k < res.length; k++) {
                        var json = self.$x2js.xml2js(res[k].data).Xml;
                        if (
                            json &&
                            ...
                            for (let i = 0; i < typeArr.length; i++) {
                                if (
                                    typeArr[i] &&
                                    tempArr.indexOf(typeArr[i]) == -1
                                ) {
                                    tempArr.push(typeArr[i]);
                                }
                            }
                        }
                    }
                    ...
                    if (min && max) {
                        let gap = Math.round(((max - min) / 4) * 1000) / 1000;

                        for (let i = 0; i < 4; i++) {
                            self.rangeTableData.push({
                                min: Math.round((min + gap * i) * 1000) / 1000,
                                max:
                                    Math.round((min + gap * (i + 1)) * 1000) /
                                    1000,
                                index: self.rangeTableData.length,
                            });
                        }
                        self.rangeTableData.push({
                            min: Math.round((min + gap * 4) * 1000) / 1000,
                            max: max,
                            index: self.rangeTableData.length,
                        });
                    }
