...
export default {
    createElement: function(self, elmentArr, options){
        ...
        var obj = Mark.createElementBox(earth, options);
        if(obj){
            elmentArr.push(obj);
        }
    },
    create: function(self){
        var that = this;
        var earth = self.stampAPI.usearth;
        Mark.createBox(earth, function(retData){
            ...
            var dataConf = deepCopy(drawConfig["box"]);
            dataConf.width.value = retData.width;
            dataConf.length.value = retData.length;
    ...
    edit: function(self, data){
        ...
        self.$refs.objectDialog.show({
            ...
            callbackOK: function(){
                ...
                var obj = Mark.editElementBox(self.elementArr[selIndex],options);
                if(obj){//保存对象到全局
