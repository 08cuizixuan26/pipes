...
export default {
  createElement(self, elmentArr, options) {
    ...
    var obj = Mark.createEleBrokenDot(earth, window.g_VideoLayer, options);
    if (obj) {
      elmentArr.push(obj);
    }
  },
    ...
    Mark.createPolyline(earth, function(retData) {
      ...
      self.$refs.objectDialog.show({
        ...
        callbackOK: function() {
          ...
          var obj = Mark.createEleBrokenDot(
            earth,
            window.g_VideoLayer,
            options
          );
  ...
  edit(self, data) {
    ...
    self.$refs.objectDialog.show({
      ...
      callbackOK: function() {
        ...
        var obj = Mark.editEleBrokenDot(self.elementArr[selIndex], options);
        if (obj) {
          //保存对象到全局
