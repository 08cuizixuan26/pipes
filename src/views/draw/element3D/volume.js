...
export default {
    createElement: function(self, elmentArr, options){
        ...
        var obj = Mark.createElementVolume(earth, options);
        if(obj){
            elmentArr.push(obj);
        }
    },
    create: function(self){
        var that = this;
        var earth = self.stampAPI.usearth;
        Mark.createVolume(earth, function(retData){
            if(retData.data == undefined){
                self.$message({
                    message: '请完成创建立体多边形绘制',
                    type: 'warning',
                    center: true
                });
                return;
            ...
            var dataConf = deepCopy(drawConfig["volume"]);
    ...
    edit: function(self, data){
        ...
        self.$refs.objectDialog.show({
            ...
            callbackOK: function(){
                ...
                var obj = Mark.editElementVolume(self.elementArr[selIndex],options);
                if(obj){//保存对象到全局
