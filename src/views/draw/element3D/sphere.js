...
export default {
    createElement: function(self, elmentArr, options){
        ...
        var obj = Mark.createElementSphere(earth, options);
        if(obj){
            elmentArr.push(obj);
        }
    },
    create: function(self){
        var that = this;
        var earth = self.stampAPI.usearth;
        Mark.createSphere(earth, function(retData){
            if(retData.location == undefined){
                self.$message({
                    message: '请至少绘制两个点创建球体',
                    type: 'warning',
                    center: true
                });
                return;
            ...
            var dataConf = deepCopy(drawConfig["sphere"]);
    ...
    edit: function(self, data){
        ...
        self.$refs.objectDialog.show({
            ...
            callbackOK: function(){
                ...
                var obj = Mark.editElementSphere(self.elementArr[selIndex],options);
                if(obj){//保存对象到全局
