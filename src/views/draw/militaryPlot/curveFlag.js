...
export default {
  createElement: function(self, elmentArr, options) {
    ...
    var obj = Mark.createElementcurveflag(earth, options);
    if (obj) {
      elmentArr.push(obj);
    }
  },
  create: function(self) {
    var that = this;
    var earth = self.stampAPI.usearth;
    Mark.createcurveflag(earth, function(retData) {
      if (!retData || retData.length < 3) {
        self.$message({
          message: "请至少绘制三个点创建曲面旗标",
          type: "warning",
          center: true
        });
        return;
      }
      var dataConf = deepCopy(drawConfig["curveFlag"]);
