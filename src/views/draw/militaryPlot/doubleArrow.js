...
export default {
  createElement: function(self, elmentArr, options) {
    ...
    var obj = Mark.createElementdoubleSArrow(earth, options);
    if (obj) {
      elmentArr.push(obj);
    }
  },
  create: function(self) {
    var that = this;
    var earth = self.stampAPI.usearth;
    Mark.createdoubleSArrow(earth, function(retData) {
      if (!retData || retData.length < 2) {
        self.$message({
          message: "请至少绘制两个点创建双箭头",
          type: "warning",
          center: true
        });
        return;
      }
      var dataConf = deepCopy(drawConfig["doubleArrow"]);
