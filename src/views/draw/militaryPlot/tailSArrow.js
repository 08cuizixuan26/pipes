...
export default {
  ...
  create: function(self) {
    var earth = self.stampAPI.usearth;
    Mark.createtailSarrow(earth, function(retData) {
      if (!retData || retData.length < 2) {
        self.$message({
          message: "请至少绘制两个点创建燕尾箭头",
          type: "warning",
          center: true
        });
      ...
      var dataConf = deepCopy(drawConfig["tailSArrow"]);
      self.$refs.objectDialog.show({
        title: "添加燕尾箭头",
        data: dataConf,
        callbackOK: function() {
          var options = {};
          options.type = "tailSArrow";
          ...
          options.rtti = StampGis.rtti_type.SE_RTTI_ELEMENT_PLOT_TAIL_SARROW;
          options.symbolId = "001";
          var obj = Mark.createElementtailSArrow(earth, options);
          if (obj) {
  ...
  edit: function(self, data) {
    ...
    self.$refs.objectDialog.show({
      ...
      callbackOK: function() {
        ...
        if (obj) {
          //保存对象到全局
          self.elementArr[selIndex].element = obj;
