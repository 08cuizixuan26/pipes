...
export default {
  createElement: function(self, elmentArr, options) {
    ...
    var obj = Mark.createElementcustomArrow(earth, options);
    if (obj) {
      elmentArr.push(obj);
    }
  },
  create: function(self) {
    var that = this;
    var earth = self.stampAPI.usearth;
    Mark.createcustomArrow(earth, function(retData) {
      if (!retData || retData.length < 2) {
        self.$message({
          message: "请至少绘制两个点创建自定义",
          type: "warning",
          center: true
        });
        return;
      }
      var dataConf = deepCopy(drawConfig["customArrow"]);
      ...
      self.$refs.objectDialog.show({
        ...
        callbackOK: function() {
          ...
          options.rtti = StampGis.rtti_type.SE_RTTI_ELEMENT_PLOT_CUSTOM_ARROW;
