...
export default {
  createElement: function(self, elementArr, options) {
    ...
    var obj = Mark.createElementequalSArrow(earth, options);
    if (obj) {
      elementArr.push(obj);
    }
  },
  create: function(self) {
    var that = this;
    var earth = self.stampAPI.usearth;
    Mark.createequalSArrow(earth, function(retData) {
      if (!retData || retData.length < 2) {
        self.$message({
          message: "请至少绘制两个点创建直箭头",
          type: "warning",
          center: true
        });
      ...
      var dataConf = deepCopy(drawConfig["equalSArrow"]);
      self.$refs.objectDialog.show({
        ...
        callbackOK: function() {
          ...
          options.rtti = StampGis.rtti_type.SE_RTTI_ELEMENT_PLOT_EQUAL_SARROW;
