...
export default {
  createElement: function (self, elementArr, options) {
    ...
    var obj = Mark.createCylinderElement(earth, window.g_VideoLayer, options);
    if (obj) {
      elementArr.push(obj);
    }
  },
  create: function (self) {
    ...
    Mark.createPoint(earth, function (retData) {
      if (!retData) {
        self.$message({
          message: '请选择一个点创建上下移动光晕',
          type: 'warning',
          center: true
        });
        return;
      }
      var dataConf = deepCopy(drawConfig["cylinderElement"]);
  ...
  edit: function (self, data) {
    ...
    self.$refs.objectDialog.show({
      ...
      callbackOK: function () {
        ...
        var obj = Mark.editCylinderElement(self.elementArr[selIndex], options, earth, window.g_VideoLayer);
        if (obj) { //保存对象到全局
