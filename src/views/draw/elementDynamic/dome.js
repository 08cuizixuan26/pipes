...
export default {
    createElement: function(self, elmentArr, options){
        ...
        var obj = Mark.createElementDome(earth, window.g_VideoLayer, options);
        if(obj){
            elmentArr.push(obj);
        }
    },
    create: function(self){
        ...
        Mark.createPoint(earth, function(retData){
            if(!retData){
                self.$message({
                    message: '请选择一个点创建穹顶光晕',
                    type: 'warning',
                    center: true
                });
                return;
            }
            var dataConf = deepCopy(drawConfig["dome"]);
    ...
    edit: function(self, data){
        ...
        self.$refs.objectDialog.show({
            ...
            callbackOK: function(){
                ...
                var obj = Mark.editElementDome(self.elementArr[selIndex], options, earth, window.g_VideoLayer);
                if(obj){//保存对象到全局
