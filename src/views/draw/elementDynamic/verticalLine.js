...
export default {
  createElement: function (self, elementArr, options) {
    ...
    var obj = Mark.createVerticalLine(earth, window.g_VideoLayer, options);
    if (obj) {
      elementArr.push(obj);
    }
  },
  create: function (self) {
    ...
    Mark.createPoint(earth, function (retData) {
      ...
      var dataConf = deepCopy(drawConfig["verticalLine"]);
  ...
  edit: function (self, data) {
    ...
    self.$refs.objectDialog.show({
      ...
      callbackOK: function () {
        ...
        var obj = Mark.editVerticalLine(self.elementArr[selIndex], options, earth, window.g_VideoLayer);
        if (obj) { //保存对象到全局
