...
export default {
    createElement: function(self, elmentArr, options){
        ...
        var obj = Mark.createElementNozzle(earth, window.g_VideoLayer, options);
        if(obj){
            elmentArr.push(obj);
        }
    },
    create: function(self){
        ...
        Mark.createPoint(earth, function(retData){
            if(!retData){
                self.$message({
                    message: '请至少选择一个点创建喷泉',
                    type: 'warning',
                    center: true
                });
                return;
            }
            var dataConf = deepCopy(drawConfig["nozzle"]);
    ...
    edit: function(self, data){
        ...
        self.$refs.objectDialog.show({
            ...
            callbackOK: function(){
                ...
                var obj = Mark.editElementNozzle(self.elementArr[selIndex], options);
                if(obj){//保存对象到全局
