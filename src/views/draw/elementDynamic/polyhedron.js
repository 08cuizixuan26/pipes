...
export default {
    createElement: function(self, elmentArr, options){
        ...
        var obj = Mark.createElementpolyhedron(earth, window.g_VideoLayer, options);
        if(obj){
            elmentArr.push(obj);
        }
    },
    create: function(self){
        ...
        Mark.createPolygon(earth, function(retData){
            ...
            for(var i =0;i<retData.length;i++){
                retData[i] = StampGis.Cartographic.fromCartesian(retData[i]);
                retData[i].longitude = StampGis.StampMath.toDegrees(retData[i].longitude);
                retData[i].latitude = StampGis.StampMath.toDegrees(retData[i].latitude);
                retData[i].altitude = retData[i].height;
                delete retData[i].height;
            }
            var dataConf = deepCopy(drawConfig["polyhedron"]);
    ...
    edit: function(self, data){
        ...
        self.$refs.objectDialog.show({
            ...
            callbackOK: function(){
                ...
                var obj = Mark.editElementpolyhedron(earth, self.elementArr[selIndex], window.g_VideoLayer, options);
                if(obj){//保存对象到全局
