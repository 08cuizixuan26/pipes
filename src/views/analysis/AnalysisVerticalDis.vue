<template>
  <Popover
    ...
    :show-header="true"
    title="垂直净距分析"
    custom-class="analysisvertic"
    :beforeClose="handleClose"
  >
    <div class="analysisvertic-container">
      <el-row>
        <el-col :span="6" class="analysisvertic-container-text">
      ...
      <el-row>
        ...
        <el-col :span="3" class="analysisvertic-container-text"
          ><span>米</span></el-col
        >
      </el-row>
      <el-row>
      ...
      <el-row>
        <el-col :span="12" class="analysisvertic-container-result">
  ...
  methods: {
    ...
    async analysisCollision() {
      ...
      for (var ni = 0; ni < this.pipeListData.length; ni++) {
        ...
        for (var i = 0; i < records.length; i++) {
          ...
          var thisFlag = true;
          if (
            parseFloat(records[i].VerticalDistance) > 0 &&
            Math.abs(parseFloat(records[i].VerticalDistance)) <
              parseFloat(records[i].VerticalISO)
          ) {
            thisClass = "bgRed";
            thisTitle = "净距<标准值";
            thisFlag = false;
          ...
          // if (parseFloat(records[i].VerticalDistance) != -999) {
          self.tableData.push({
            id: tempid, //getNameNoIgnoreCase("US_KEY", 1, true)
            layer: self.pipeListData[ni].name,
            VerticalDistance:
              parseFloat(records[i].VerticalDistance) == -999
                ? "-"
                : parseFloat(records[i].VerticalDistance).toFixed(2),
            VerticalISO:
              parseFloat(records[i].VerticalISO) == -999
                ? "-"
                : parseFloat(records[i].VerticalISO).toFixed(2),
    ...
    download() {
      ...
      import("@/vendor/Export2Excel").then((excel) => {
        const tHeader = ["编号", "图层", "垂直净距", "标准"];
        const filterVal = ["id", "layer", "VerticalDistance", "VerticalISO"];
...
.analysisvertic-container {
  height: 550px;

  .analysisvertic-container-text {
    height: 30px;
    line-height: 30px;
    font-size: 14px;
    text-align: left;
  ...
  .analysisvertic-container-result {
    height: 30px;
    line-height: 30px;
    font-weight: 700;
    text-align: left;
  ...
  .analysisvertic-container-table {
    margin-bottom: 0px;
  }
}
