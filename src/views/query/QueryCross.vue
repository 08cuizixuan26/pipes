  ...
  methods: {
    ...
    handleNodeClick(data) {
      ...
      queryFunc(params).then(function (res) {
        ...
        if (json && json.Result && json.Result.Record) {
          ...
          for (let i = 0; i < records.length; i++) {
            if (
              roadArr.indexOf(records[i].name) == -1 &&
              records[i].name != data.name
            ) {
              roadArr.push(records[i].name);
              self.treeData1.push(records[i]);
            }
          }
        }
      });
    ...
    createBuffer(linePoints) {
      ...
      for (let i = 0; i < lastPolygon.length; i++) {
        earth.document.elementRoot.detach_object(lastPolygon[i]);
      }
      lastPolygon = [];
      var polygonArray =
    ...
    handleNodeClick1(data) {
      ...
      queryFunc(params)
        .then(function (res) {
          var json = self.$x2js.xml2js(res.data).Xml;
          if (json && json.Result && json.Result.Record && json.Result.Record) {
            let records = json.Result.Record;
            Array.isArray(records) || (records = [records]);

            self.point = records[0].SHAPE.Point.Coordinates;
            self.createCircleObj(self.point);
          }
        })
        .finally(() => {
          self.loading = false;
    ...
    createCircleObj(pointStr) {
      ...
      self.sc = `(3,1,${Number(self.inputRadius)},${pointStr},0)`;

      var pos = StampGis.Cartesian3.fromDegrees(
        Number(pointStr.split(",")[0]),
        Number(pointStr.split(",")[1]),
        0
      );
      let circle = earth.Factory.CreateElementCircle({
        name: "name",
        doc: earth.document,
      });

      circle.BeginUpdate();
      circle.setRadiusAndCenter(Number(self.inputRadius), pos);
      circle.set_altitude_type(1);
      circle.lineColor = 0xffff0000;
      ...
      lastPolygon.push(circle);

      var tmpPoints = [];
      var tempPoint = new StampGis.Cartographic(
        circle.transform.longitude,
        circle.transform.latitude,
        0
      );
      tmpPoints.push(tempPoint);
      ...
      StampGis.when(promise, function () {}).then(() => {
        var pos = StampGis.Cartesian3.fromRadians(
          tmpPoints[0].longitude,
          tmpPoints[0].latitude,
          tmpPoints[0].height + Number(self.inputRadius) * 3
        );
        earth.application.observer.flyTo({
          destination: pos,
          orientation: {
