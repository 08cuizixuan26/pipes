    ...
    export default {
        ...
        methods: {
            ...
            close(){
                var tempPath = '';
                if(this.selRow){
                    tempPath = this.reqPath + this.selRow.name;
                }else{
                    tempPath = this.reqPath.substr(0,this.reqPath.length-1);
                }
            ...
            getFilePathList(tempPath){
                var self = this;
                GetFilePath(tempPath,'').then(resp => {
                    var d = self.$x2js.xml2js(resp.data);
                    
                    self.selRow = null;
                    self.tableData = [];
                    self.reqPath = tempPath;
                    if(d && d.xml && d.xml.publishtool){ 
                        if(d.xml.publishtool.path){
                            var value = d.xml.publishtool.path;
                        ...
                        if(d.xml.publishtool.file){
                            var files = d.xml.publishtool.file;
